...
var MODELS = {
  ...
  "conceptualise an ~ imageable thing ~ I that has the value V as ~ image ~",
    ...
    "there is a stage named 'JOHN PEEL STAGE' that is in the location 'Main Stages'.",
  ]
}
...
//Node application that gets real time tweets

console.log('The bot is starting');

var Twit = require('twit'); //Import the twitt package
var config = require('./config'); //require config --> the api keys
var fs = require('fs');
...
var myModel = [
  ...
  "there is a tell policy named 'p2' that has 'true' as enabled and has the agent 'agent1' as target",
  "there is a listen policy named 'p4' that has 'true' as enabled and has the agent 'agent1' as target"

];
//var node = new cenode.CENode(cenode.MODELS.CORE);
var node = new cenode.CENode(cenode.MODELS.CORE,myModel);
//node.agent.set_name("agent2S");
var T = new Twit(config); //create an instance of Twit

var perfomers = ["Florence and the Machine",
"The Libertines",
"Motorhead",
"Mary J Blige",
"Jamie xx"];

var stages = ["PYRAMID STAGE",
"OTHER STAGE",
"WEST HOLTS",
"THE PARK STAGE",
"JOHN PEEL STAGE"];

function getNameExact(tweet,option){

  for(var i = 0; i<option.length;i++){
    var inputString = option[i];
    //console.log(inputString);
    inputString = "\\b" + inputString.replace(" ", "\\b \\b") + "\\b";
    if(tweet.toLowerCase().match(inputString.toLowerCase())==null){
...
function getNameToken(tweet,option){
  ...
  //console.log(tweetTokenised); //testing purposes
  var distances = [];
  ...
  for(i=0;i<tweetTokenised.length;i++){
    ...
    var temp = [];
    //compare token with list of artists performing on option
    for(j=0;j<option.length;j++){
      //remove whitespace from the tweet tokens
      //console.log(tweetTokenised[i]+ "--->"+option[j]); //testing purposes
      var score = natural.LevenshteinDistance(tweetTokenised[i].replace(/\s+/g, '').toLowerCase(),option[j].replace(/\s+/g, '').toLowerCase());
  ...
  if(distances[0] == null){
    ...
    return distances[0].key; //get artist name with the lowest score
  }
  
}

function getTime(tweet){
  var playingWords = ["playing now" , "about to go on" , "on stage now" , "ready" , "waiting"];
  for(i=0;i<playingWords.length;i++){
    //check for if the band is on stage at the time of the tweet.
    var current = playingWords[i];
    //console.log(current);
    current = "\\b" + current.replace(" ", "\\b \\b") + "\\b";
    //console.log(current);
    if(tweet.toLowerCase().match(current.toLowerCase())!=null){ //if there is a match
      //place holder
      //time = (tweets[i]["time"]);
      ...
      //use regex to get time 
      var re = /(?:[0][0-9]|[1][012])(?::[0-5][0-9])?(?:am|pm)|(?:[0-1][0-9]|2[0-3]):(?:[0-5][0-9])/g;
      var arr = re.exec(tweet);
      if(arr!= null){
        return arr[0];
      }
    }
...
function addToNode(sentenceToAdd){
  node.add_ce(sentenceToAdd);
...
function getTweets(){
  T.get('search/tweets', { q: '#fyp2016test', count: 100 }, function(err, data, response) {
      //console.log(data)
      ...
      for(var i=0; i<tweets.length;i++){
        ...
        var band = getNameExact(text,perfomers);
        if(band == null){
          band = getNameToken(text,perfomers);
        }
        if(band == null) 
          continue;
        //console.log(band);

        var stage = getNameExact(text,stages);
        if(stage == null){
          stage = getNameToken(text,stages);
        }
        ...
        if(time == null){
          console.log("in here");
          time = new Date(Date.parse(tweets[i].created_at));
        ...
        //console.log(hour);
        //console.log(minutes);
        //var timeOfPerfomance = hour + ":" + minutes;
        //console.log(time); 

        var sentenceToAdd  = "there is a band named '" + band + "' that plays at the time '"  + time  + "' and that has the stage '" + stage + "' as venue." 
        console.log(sentenceToAdd);
        node.add_ce(sentenceToAdd);
        
        //var nodeResult = add_ce(sentenceToAdd);
...
var stream = T.stream('statuses/filter', { track: '#fyp2016test' });
  stream.on('tweet', function(tweet){
  var datetime = new Date(Date.parse(tweet['created_at'])).toISOString().replace(/T/, ' ').replace(/\..+/,''); //get date
  ...
  if(tweet.coordinates.coordinates[1] != null){
    var longitude = tweet.coordinates.coordinates[1];
  }
  //console.log(latitude);
  //console.log(longitude);
  
  console.log(tweet.text,datetime,latitude,longitude);
});*/
